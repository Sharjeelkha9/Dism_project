...
if(isset($_POST['orderPlace'])){
    ...
    if ($order_type === "Home Delivery" && empty($uaddress)) {
        ...
        foreach ($_SESSION['cart'] as $key => $val) {
            ...
            $productId = str_pad($val['proid'], 7, "0", STR_PAD_LEFT);
            $randomOrderNum = mt_rand(10000000, 99999999);
            $orderNumber = $orderTypeDigit . $productId . $randomOrderNum;
            ...
            $checkStock = $pdo->prepare("SELECT quantity FROM `products` WHERE `id` = :pid");
            $checkStock->bindParam("pid", $val['proid']);
            $checkStock->execute();
            $stockData = $checkStock->fetch(PDO::FETCH_ASSOC);

            if ($stockData) {
                $currentStock = $stockData['quantity'];
                if ($currentStock >= $val['proquantity']) {
                    // Reduce product quantity in the database
                    $updateQtyQuery = $pdo->prepare("UPDATE `products` SET `quantity` = `quantity` - :pq WHERE `id` = :pid");
                    $updateQtyQuery->bindParam("pq", $val['proquantity']);
                    $updateQtyQuery->bindParam("pid", $val['proid']);
                    if ($updateQtyQuery->execute()) {
                        echo "<script>console.log('Product ID {$val['proid']} quantity updated successfully.');</script>";
                    } else {
                        echo "<script>console.log('Failed to update quantity for Product ID {$val['proid']}');</script>";
                    }
                } else {
                    echo "<script>alert('Not enough stock for {$val['proname']}. Available: $currentStock, Requested: {$val['proquantity']}');</script>";
                }
            } else {
                echo "<script>console.log('Product ID {$val['proid']} not found in database');</script>";
            }
        }
